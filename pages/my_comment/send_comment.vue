	...
	<view>
		<view class="send-box uni-flex">
			<view class="feedback-star-view">
				...
				<text class="feedback-star" v-for="(value,key) in stars" :key="key" :class="key < temp.start ? 'active' : ''" @tap="chooseStar(value)"></text>
			</view>
		...
		methods: {
			...
			chooseImage: async function() {
			    ...
			    uni.chooseImage({
			        ...
			        count: this.temp.imageList.length + this.count[this.countIndex] > 9 ? 9 - this.imageList.length :
