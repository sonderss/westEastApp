		...
		<view class="content_dynamics">
			...
			<view class="banner">
				<swiper :duration="200" @change="change">
					<swiper-item class="swiper-item" v-for="(x,index) in list.bigimgList" :key="index">
		...
		<view class="talk">
			...
			<view>
				<view v-if="list.pinglun==0?false:true">
					<talk-list :list="talklist" @updateDianzan="updateDianzan" @updatereplyzan="updatereplyzan" @replaybozhu="replaybozhu"></talk-list>
				</view>
				<view v-else>
					<view class="notalk">
...
<script>
	...
	export default{
		...
		methods:{
			...
			goHuati(){
				uni.navigateTo({
					 url:'../../pages/cjq_testpages/index?isP='+1+'&title=#让选材没有秘密#&ptitle=12354 围观'
				})
			},
			goQuanzi(){
				uni.navigateTo({
					 url:'../../pages/cjq_testpages/index?isP='+1+'&title=装修日记&ptitle=126住友围观'
				})
			},
			//分享
			share_weixin(){
