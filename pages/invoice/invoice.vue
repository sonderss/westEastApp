		...
		<view class="invoiceBox">
				...
				<view class="invoiceType invoiceInfoBox">
					...
					<view class="invoiceCategory">
						<block v-for="(item,index) in invoiceCategory" :key="index">
							<radio-group @change="radioChange"><label class="radio"><radio :value="item.type" :checked="index === current" style="transform:scale(0.7)" color="#f00" />{{item.txt}}</label></radio-group>
						</block>					
					</view>
			...
			<uni-popup ref="popup" type="bottom" :custom="false" :radius="true">
				<view class="bill-type" v-if="contentInfo">
					...
					<view class="cancel uni-flex" @click="cancel"><text>取消</text></view>
				</view>
...
<style>
	...
	.internet text.curr{color:#f00;border:1px solid #f00;}
	.noinvoice{padding:0 13upx;margin-top:56upx;}
	.sure{width:100%;height:74upx;line-height:74upx;color:#fff;font-size:28upx;text-align:center;background:#f00;display:block;border-radius:37upx;}
	...
	.bill-type view.cancel{color:#f00;font-size:36upx;}
