	...
	<view>
		...
		<view class="header">
			<view>
				...
				<view class="contents icon">					
					<view v-for="item in currentcity" :key='item' @tap="choosecity(item)">
						<text class="current">
							{{item}}
						</text>
					</view>
		...
		<uni-collapse >
		    <uni-collapse-item v-for="(key,index) in listData" :key="index" :title="index">
		        <view class="contents">
					<view v-for="(x,index) in key.citylist" :key="index" @tap="choosecity(x)">
						<text>{{x}}</text>
					</view>
				</view>
		    </uni-collapse-item>
...
<script>
	...
	export default{
		data(){
			return{
				listData: {},
				hotcity:['全部','上海','北京','深圳','南京','天津','福州','成都','武汉','安徽','昆明','西安'],
				currentcity:['广州'],
				flag:false
			}
		},
		...
		methods:{
			...
			back(){
				...
				// this.$store.commit('renfter',this.$store.state.changedCity) 
			},
			objKeySort(obj){
			...
			choosecity(res){
				...
				// console.log(res)
				if(this.flag){
					this.$store.commit('choose',res)
					console.log(111)
				}else{
					this.$store.commit('mycity',res)
					console.log(222)
				}
				let pages = getCurrentPages(); 
				let nowPage = pages[ pages.length - 1]; 
				let prevPage = pages[ pages.length - 2 ];  
				prevPage.$vm.city = res;
				uni.navigateBack({
					delta: 1
				});
