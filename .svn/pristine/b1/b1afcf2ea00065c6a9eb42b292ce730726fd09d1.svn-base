<template >
	<view class="body" >
			...
			<view class="shop_id">
				<view class="shop_id1" v-for="(item,i) in shop_goods_list" :key="i">
					<checkbox-group @change="all_checkboxChange($event,i)"  >
						<label>
							<checkbox  color="#d81e06" :checked="shop_select_all[i]"/>
							<text class="shopname">
								{{item.shopname}}
							</text>
						</label>
						...
						<view class="uni-list-cell uni-list-cell-pd" v-for="(item2,j) in item.goods_list" :key="j">
							<view class="goods_flex">
								<label class="goods_flex">
									<checkbox color="#d81e06" :checked="item2.checked" :value="i+'_'+j"  />
								...
								<view class="goods_attribute">
									...
									<view class="attribute" v-for="(item3,k) in item2.parameter" :key="k">{{k}}:{{item3}}</view>
		...
		<view v-else class="no_goods">
			...
			<view class="go_store">去逛逛</view>
		</view>
		<guess-like v-if="goods.like.length > 0" :content="goods.like"></guess-like>
		...
		<view class="total_box" :class="total_box_margin?'margin100':''" v-if="shop_goods_list.length>0">
			<checkbox-group  @change="total_all_checkboxChange" >
				<label class="select_all">
					<checkbox color="#d81e06" :checked="total_all_select" />
					<text>全选</text>
				</label>
			</checkbox-group>
			<view class="order_btn" v-if="!is_edit">
				<text>合计：</text>
				<text style="color: #ff0000;">{{all_total_price}}</text>
