<template>
	<view>
		...
		<!-- <view class="emoji-box" :class="showEmji" v-if="showEmji" @touchmove.stop.prevent="discard" >
			<swiper class="swiper" indicator-dots="true" duration="150">
				<swiper-item >
					<view v-for="(page,pid) in listEmoji"  :key="pid" @tap="addEmoji(page)">
		...
		<view class="input-box" :class="showEmji"  @touchmove.stop.prevent="discard">
			...
			<!-- #ifndef H5 -->
			<view class="voice">
				<view class="icon" :class="isVoice?'yuyin':'jianpan'" @tap="switchVoice"></view>
			</view>
			<!-- #endif -->
			<!-- #ifdef H5 -->
			...
			<view class="textbox">
				...
				<view class="text-mode"  :class="isVoice?'hidden':''">
					<view class="box">
						<textarea auto-height="true" v-model="textMsg" id="textMsg"  ref='chatInput' @focus="isFocus" :adjust-position="bool" cursor-spacing='140'   />
					</view>
		...
		<!-- <loading :loading='showLoading' v-if="isLoading"></loading> -->
	</view>
</template>

<script>
import '../../static/iconfont/font.scss'
import loading from '../../components/nairenk-waterfall-flow/nairenk-waterfall-flow'
	export default {
		components:{loading},
		data() {
			return {
	...
	// 	},
		methods:{
			isFocus(e){
				//检测获取到焦点时，键盘弹出然后在操作键盘外空间时，键盘隐藏
				 if(this.test === 1){
				     ...
				     this.showEmji = this.showEmji=='showEmoji'?'fousEm0':'';
			...
			getMsgList(){
				// 消息列表
				let list = [
					{id:0,uid:0,username:"大黑哥",face:"/static/img/message/toux.png",time:"12:56",type:"text",msg:{content:"为什么没有可口可乐？"}},
					{id:1,uid:1,username:"售后客服008",face:"/static/img/message/touxiang.png",time:"12:57",type:"text",msg:{content:"因为青椒炒饭比较好吃"}},
				]
				// 获取消息中的图片,并处理显示尺寸
			...
			sendMsg(content,type){
				...
				lastid++;
				let msg = {id:lastid,uid:0,username:"大黑哥",face:"/static/img/message/toux.png",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
				this.screenMsg(msg);
				// 定时器模拟对方回复,三秒
				setTimeout(()=>{
					lastid = this.msgList[this.msgList.length-1].id;
					lastid++;
					msg = {id:lastid,uid:1,username:"售后客服008",face:"/static/img/message/touxiang.png",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
					this.screenMsg(msg);
				},3000)
			},
