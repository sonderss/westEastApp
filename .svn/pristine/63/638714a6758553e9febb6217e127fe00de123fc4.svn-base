<template>
	<view>
		<view class="ed-textare-box">
			<textarea class="ed-textare" :value='value' placeholder="快来和大家分享故事吧!" />
			</view>
		<view class="ed-topic-box">
			<view class="uni-panel">
				<view class="uni-panel-h">
					...
					<view @tap="jumpTopic()">
						<text style="font-size: 24upx;color: #A8A8A8;">{{list.topic}}</text>
			...
			<view class="uni-panel">
				<view class="uni-panel-h">
					...
					<view @tap="jumpCircle()">
						<text style="font-size: 24upx;color: #A8A8A8;">{{list.cricle}}</text>
						<text class="uni-panel-icon uni-icon">&#xe470;</text>
					</view>
				</view>
			</view>
...
<script>
	...
	export default{
		...
		onLoad(res) {
			// console.log(res);
			if(res.topic){
				this.list.topic = res.topic
			}
			if(res.cricle){
				this.list.cricle = res.cricle
			}
		...
		methods:{
			...
			jumpTopic:function(){
				uni.navigateTo({
					url: '../topics/all_topic'
				});
			},
			jumpCircle:function(){
				uni.navigateTo({
					url: '../cricle/select_cricle'
				});
			},
...
<style scoped>
	...
	.ed-textare{font-size:26upx;margin-left:30upx;margin:30upx 0 0 30upx}
